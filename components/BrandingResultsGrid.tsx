...
const LoadingSpinner = () => (
    <div className="flex flex-col items-center justify-center gap-4">
        ...
        <div className="flex flex-col items-center">
            <span className="text-[10px] font-black text-accent uppercase tracking-[0.3em]">Processing</span>
            <span className="text-[8px] font-medium text-text-muted uppercase tracking-widest mt-1">Refining Neurals</span>
        </div>
    </div>
);
...
const ImageResultCard: React.FC<{ 
    ...
    const id = String(result.category || 'Visual Asset');
    ...
    return (
        <div className="flex flex-col gap-4 group/card animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="relative w-full aspect-square bg-[#050505] rounded-[2rem] overflow-hidden border border-white/5 transition-all duration-500 shadow-2xl group-hover/card:shadow-accent/10 group-hover/card:border-accent/30 box-content">
                {(result.isLoading || result.isEditing) ? (
                    ...
                    <>
                        ...
                        />
                        <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-500">
                           <div className="flex flex-col gap-1">
                              <span className="text-[10px] font-black text-accent uppercase tracking-widest">{id}</span>
                              <span className="text-[8px] text-white/50 font-medium uppercase truncate">Photorealistic Composition • 300 DPI</span>
                           </div>
                        </div>
                    </>
                ) : result.error ? (
                ...
                <div className="flex flex-col gap-4 px-1">
                    <div className="relative group/edit">
                        <input 
                            ...
                            placeholder="Refine visual details..."
                            className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-xs text-white focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all placeholder:text-text-muted font-medium"
                            onKeyDown={(e) => e.key === 'Enter' && handleApplyEdit()}
                        />
                        <button 
                            onClick={handleApplyEdit}
                            disabled={!localPrompt.trim()}
                            className="absolute right-1.5 top-1.5 bottom-1.5 px-4 bg-accent hover:bg-accent-dark text-white text-[9px] font-black rounded-xl transition-all disabled:opacity-0 disabled:scale-90 shadow-xl shadow-accent/20"
                        >
                            EVOLVE
                        </button>
                    </div>
                    ...
                    <div className="flex gap-2">
                        <button 
                            ...
                            className="btn-secondary flex-1 py-3 rounded-xl border-white/5 text-[9px] font-black tracking-widest"
                        >
                            2K MASTER
                        </button>
                        <button 
                            ...
                            disabled={isDownloading} 
                            className="btn-primary flex-1 py-3 rounded-xl shadow-none text-[9px] font-black tracking-widest"
                        >
                            4K CINEMATIC
                        </button>
                    </div>
                ...
                <div className="fixed inset-0 bg-black backdrop-blur-3xl flex items-center justify-center z-[99999] p-4 animate-in fade-in duration-500 cursor-zoom-out" onClick={() => setIsFullViewOpen(false)}>
                    <div className="relative w-full h-full flex items-center justify-center p-8 md:p-12 lg:p-24">
                        ...
                        <div className="absolute top-10 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/40 px-6 py-3 rounded-full border border-white/10 backdrop-blur-xl">
                            <span className="text-[10px] font-black text-white uppercase tracking-[0.4em]">{id}</span>
                            <div className="w-1 h-3 bg-accent rounded-full"></div>
                            <span className="text-[9px] font-bold text-white/40 uppercase tracking-widest">Master Preview</span>
                        </div>
                    </div>
                </div>,
                document.body
